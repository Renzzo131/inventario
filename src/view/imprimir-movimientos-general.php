...
<head>
  <meta charset="UTF-8">
  <title>Registro general de movimientos</title>
  <style>
    body {
...
<br><br>
  ...
  <p>INFORME <span class="subrayado">:  Relación de Movimientos Registrados</span></p>

  <p class="motivo">DESCRIPCIÓN: <span class="subrayado"> Listado consolidado de movimientos registrados en el sistema institucional.</span></p>
  ...
  <table style="width: 100%; border-collapse: collapse; margin-top: 20px;" cellspacing="0" cellpadding="4">
    <thead>
      <tr>
        ...
        <th style="border: 0.3px solid #444; background-color: #f2f2f2;">AMBIENTE DE ORIGEN</th>
        <th style="border: 0.3px solid #444; background-color: #f2f2f2;">AMBIENTE DE DESTINO</th>
        <th style="border: 0.3px solid #444; background-color: #f2f2f2;">USUARIO REGISTRANTE</th>
        <th style="border: 0.3px solid #444; background-color: #f2f2f2;">FECHA DE REGISTRO</th>
        <th style="border: 0.3px solid #444; background-color: #f2f2f2;">DESCRIPCIÓN</th>
    ...
    <tbody>
        ...
        ';
     if (empty($respuesta->movimientos)) {
      $contenido_pdf .= '<tr>
...
foreach ($respuesta->movimientos as $movimiento) {
    ...
    $contenido_pdf .= '<td style="border: 0.3px solid #444;">'.$movimiento->ambiente_origen.'</td>';
    $contenido_pdf .= '<td style="border: 0.3px solid #444;">'.$movimiento->ambiente_destino.'</td>';
    $contenido_pdf .= '<td style="border: 0.3px solid #444;">'.$movimiento->registrante.'</td>';
    $contenido_pdf .= '<td style="border: 0.3px solid #444;">'.$movimiento->fecha.'</td>';
    $contenido_pdf .= '<td style="border: 0.3px solid #444;">'.$movimiento->descripcion.'</td>';
    $contenido_pdf .= '<td style="border: 0.3px solid #444;">'.$movimiento->institucion.'</td>';
    $contenido_pdf .= '</tr>';
    $contador++;
}
     }
...
$pdf->SetTitle('Listado general de movimientos');
...
$pdf->Output('Listado de movimientos.pdf', 'I');
