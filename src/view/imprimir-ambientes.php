<?php
    ...
    curl_setopt_array($curl, array(
        CURLOPT_URL => BASE_URL_SERVER."src/control/Ambiente.php?tipo=buscar_ambientes&sesion=".$_SESSION['sesion_id']."&token=".$_SESSION['sesion_token'], //url a la que se conecta
...
<head>
  <meta charset="UTF-8">
  <title>Registro general de ambientes</title>
  <style>
    body {
...
<body>
  <h2><i>LISTADO GENERAL DE AMBIENTES REGISTRADOS</i></h2>
<br><br>
  ...
  <p>INFORME <span class="subrayado">:  Relación de Ambientes Registrados</span></p>

  <p class="motivo">DESCRIPCIÓN: <span class="subrayado"> Listado consolidado de ambientes registrados en el sistema institucional.</span></p>
  ...
  <table style="width: 100%; border-collapse: collapse; margin-top: 20px;" cellspacing="0" cellpadding="4">
    <thead>
      <tr>
        ...
        <th style="border: 0.3px solid #444; background-color: #f2f2f2;">INSTITUCIÓN</th>
        <th style="border: 0.3px solid #444; background-color: #f2f2f2;">ENCARGADO</th>
        <th style="border: 0.3px solid #444; background-color: #f2f2f2;">CÓDIGO</th>
        <th style="border: 0.3px solid #444; background-color: #f2f2f2;">DENOMINACIÓN DEL AMBIENTE</th>
        <th style="border: 0.3px solid #444; background-color: #f2f2f2;">OTROS DETALLES</th>
      </tr>
    </thead>
    <tbody>
        
        ';
     if (empty($respuesta->ambientes)) {
      $contenido_pdf .= '<tr>
...
foreach ($respuesta->ambientes as $ambiente) {
    ...
    $contenido_pdf .= '<td style="border: 0.3px solid #444;">'.$ambiente->nombre_institucion.'</td>';
    $contenido_pdf .= '<td style="border: 0.3px solid #444;">'.$ambiente->encargado.'</td>';
    $contenido_pdf .= '<td style="border: 0.3px solid #444;">'.$ambiente->codigo.'</td>';
    $contenido_pdf .= '<td style="border: 0.3px solid #444;">'.$ambiente->detalle.'</td>';
    $contenido_pdf .= '<td style="border: 0.3px solid #444;">'.$ambiente->otros_detalle.'</td>';
    $contenido_pdf .= '</tr>';
    $contador++;
}
     }
...
$pdf->SetTitle('Listado general de ambientes');
...
$pdf->Output('Listado de ambientes.pdf', 'I');
