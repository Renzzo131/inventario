...
if (!isset($respuesta->ambientes) || !is_array($respuesta->ambientes)) {
    echo "Error al obtener los ambientes.";
...
$fila = 1;
$headers = ['N°','Institución', 'Encargado', 'Código', 'Detalle', 'Otros detalles'];
...
foreach ($respuesta->ambientes as $ambiente) {
    ...
    $activeWorksheet->setCellValue('B'.$fila, $ambiente->nombre_institucion);
    $activeWorksheet->setCellValue('C'.$fila, $ambiente->encargado);
    $activeWorksheet->setCellValue('D'.$fila, $ambiente->codigo);
    $activeWorksheet->setCellValue('E'.$fila, $ambiente->detalle);
    $activeWorksheet->setCellValue('F'.$fila, $ambiente->otros_detalle);
    $contador++;
    $fila++;
}
foreach (range('A', 'F') as $col) {
...
header('Content-Disposition: attachment;filename="Ambientes.xlsx"');
